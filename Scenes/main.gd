...
func crear_npc(pos: Vector2):
	...
	var tiene = Simbolos.simbolos_random_de(dominio_propio, 2).map(func (s): return s['imagen'])
	var quiere = Simbolos.simbolos_random_de(dominio_quiere, 1).map(func (s): return s['imagen'])
